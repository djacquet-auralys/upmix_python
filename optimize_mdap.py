...
from vbap import vbap_2d, mdap_2d
...
def compute_spatialization_score(width ,spat_delays = spat_delays, target_width = 30, seats=seats,speaker_pts=spk,sources=source, speaker_axis = speaker_axis, directivity=directivity, plot=False, print_flag = True, color = 'b'):
    ...
    gains  = mdap_2d(speaker_pts,central_seat, sources, width = width)
    ...
    # jnd_theta = 1.5 #just noticeable difference of localization
    # jnd_w = 0.05 #just noticeable difference of width
    # λ = jnd_theta/jnd_w/180
    # λ = 20

    # dw = (w - target_width)/180
    ...
    # angle_diff_score = 10*np.mean(angle_diff**2) + 2*np.mean((w-target_width)**2)
    ...
    if plot:
        ...
        plt.title('score:' + str(angle_diff_score) +', width:' + str(np.mean(w)))
...
#%%
width_to_test = np.arange(0., .4, 0.02)
target_width = np.arange(10,50,10)
...
for k in range(len(target_width)):
    ...
    for i in range(len(width_to_test)):
        angle_dev[k,i] =  compute_spatialization_score(width_to_test[i],target_width = target_width[k], spat_delays = spat_delays, plot=True, print_flag = False)
...
# plt.plot(width_to_test, angle_dev)
